...
pygame.display.set_caption("Air Traffic Control (ATC)") # Display title name for window
BLUE = (0, 0, 255)
WHITE = (122,255,0)
BLACK = (0, 0, 0)
RED = (255, 0, 0)
...
clist = [RED, AFB, BLUE, AC, AO, AG, BRG] # list of colors of plane
timegap = [0]

def fangle(f): # calculates the angle of plane wrt centre
    xc = 350
    yc = 300
...
def drawBigc(): # draws a bigger circle to denote the path where the planes move
...
p = 0
i = 0
def planeMove1(aa,ab): # defines movement function for planes from starting point
    if(len(planeList) == 0):
        ...
        while (((int(math.sqrt(((xi-350)**2)+((yi-300)**2)))) > 150)):
            ...
            for j in range(0,l):
                if plist[j - 1] == 0:
                    ...
                    if dist < 10:
                        c = c + 0.0001*(slist[len(slist)-1]/1500)
                        continue
                else:
        ...
        global p 
        p += 1
        print "Plane",p," is beginning to land"
        if cont: # condition for landing
            while (int(y2) != 300):
                #ct=ct+1
                window.fill(WHITE)
                drawBigc()
...
def inspd(): # speed input of plane
...
def mouseClick(aa,ab): # function for responding to mouse click in window
...
while True: #Now the mouse click thread starts
    mouseClick("True",1)
